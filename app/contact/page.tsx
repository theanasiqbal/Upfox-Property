...
function ContactForm() {
    ...
    // Pre-fill inquiry type from URL query param
    if (typeof window !== 'undefined') {
        const params = new URLSearchParams(window.location.search);
        const type = params.get('type');
        if (type && !formData.inquiryType) {
            setFormData((f) => ({ ...f, inquiryType: type }));
        }
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSubmitted(true);
            setFormData({ name: '', phone: '', email: '', message: '', propertyInterest: '', inquiryType: '' });
        } catch { } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="bg-gray-50 dark:bg-white/5 rounded-2xl p-8 border border-gray-100 dark:border-white/10 shadow-sm dark:shadow-none">
            ...
            {submitted && (
                <div className="bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 text-green-700 dark:text-green-400 px-4 py-4 rounded-xl mb-6 flex items-center gap-3">
                    <CheckCircle className="w-5 h-5 flex-shrink-0" />
                    <span className="text-sm font-medium">Thank you! Your inquiry has been submitted. We&apos;ll get back to you within 24 hours.</span>
                </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {/* Property Interest */}
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Property Type of Interest</label>
                    <select
                ...
                <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-3.5 btn-gradient font-semibold rounded-xl flex items-center justify-center gap-2 text-sm disabled:opacity-60 hover:scale-[1.02] transition-all duration-300"
                >
                    {loading ? (
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-3 mb-10">
                                    <a
                                        ...
                                        className="inline-flex items-center gap-2 px-6 py-3 bg-dark-blue hover:bg-dark-blue-dark text-white font-semibold rounded-full text-sm transition-all hover:scale-105"
                                    >
                                    ...
                                    <a
                                        ...
                                        className="inline-flex items-center gap-2 px-6 py-3 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-full text-sm transition-all hover:scale-105"
                                    >
