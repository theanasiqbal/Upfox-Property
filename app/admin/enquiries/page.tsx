...
export default function AdminEnquiriesPage() {
    ...
    const handleUpdateStatus = async (id: string, newStatus: string) => {
        try {
            ...
            if (res.ok) {
                setEnquiries(prev => prev.map(e => e._id === id ? { ...e, status: newStatus as Enquiry['status'] } : e));
            }
        } catch (error) {
    ...
    const handleExport = async (filters: ExportFilters) => {
        ...
        try {
            ...
            exportToExcel(rows, columns, `enquiries-${today}`);
        } catch (err) {
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <form onSubmit={handleSearch} className="flex-1 flex gap-2">
                    ...
                    <button
                        type="submit"
                        className="px-4 py-2.5 bg-accent-purple text-white rounded-xl text-sm font-medium hover:bg-accent-purple/90 transition-colors"
                    >
                        Search
                    </button>
                    {search && (
                        <button
                            ...
                            className="px-3 py-2.5 border border-gray-200 dark:border-white/10 rounded-xl text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
                        >
                            Clear
                        </button>
                    )}
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-50 dark:bg-white/5 border-b border-gray-100 dark:border-white/10">
                                <tr>
                                    <th
                                        className="px-6 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:text-accent-purple"
                                    ...
                                    <tr key={enquiry._id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap">
                                            <a
                                                ...
                                                className="font-mono text-xs px-2 py-1 bg-gray-100 dark:bg-white/10 rounded hover:bg-accent-purple/10 hover:text-accent-purple dark:hover:text-accent-purple transition-colors"
                                        ...
                                        <td className="px-6 py-4 text-sm whitespace-nowrap">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {enquiry.status !== 'new' && (
                                                    <button
                                                        onClick={() => handleUpdateStatus(enquiry._id, 'new')}
                                                        className="p-1.5 text-gray-400 hover:text-gray-800 hover:bg-gray-100 dark:hover:bg-gray-800 dark:hover:text-white rounded-lg transition-colors"
                                                        title="Re-open"
                                                    >
